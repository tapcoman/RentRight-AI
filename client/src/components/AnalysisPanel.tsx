        ...
        <div className="px-6 pb-6">
          <div className="space-y-4">
            {(results?.insights || []).map((insight: any, index: number) => {
              const isHighPriority = insight.severity === 'high' || insight.type === 'warning';
              const isMediumPriority = insight.severity === 'medium';
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-start gap-3">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                        isHighPriority ? 'bg-red-100' : isMediumPriority ? 'bg-slate-100' : 'bg-green-100'
                      }`}>
                        <svg className={`w-4 h-4 ${
                          isHighPriority ? 'text-red-600' : isMediumPriority ? 'text-slate-600' : 'text-green-600'
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full font-medium ${
                      isHighPriority ? 'bg-red-100 text-red-800' : isMediumPriority ? 'bg-slate-100 text-slate-800' : 'bg-green-100 text-green-800'
                    }`}>
                      {isHighPriority ? 'High Priority' : isMediumPriority ? 'Medium Priority' : 'Good'}
                    </span>
                  </div>
                  ...
                  {insight.recommendation && (
                    <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                      <div className="flex items-start gap-2">
                        <div className="w-4 h-4 bg-orange-200 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                          <svg className="w-2 h-2 text-orange-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                          </svg>
                        </div>
                        <div>
                          <h5 className="font-medium text-orange-800 text-sm mb-1">Recommendation:</h5>
