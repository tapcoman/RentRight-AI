        ...
        <div className="bg-white rounded-lg p-4 border border-[#F3EEE4] shadow-sm">
          ...
          <div className="relative">
            ...
            <Input 
              ...
              className={`pl-10 ${emailError ? "border-red-500 focus:ring-red-500" : "border-orange-100 focus:border-orange-300 focus:ring-orange-200"}`}
              required
            />
          </div>
          {emailError && (
...
export default function PaymentModal({ isOpen, onClose, onSuccess, documentId }: PaymentModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-xl shadow-lg z-50 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="bg-gradient-to-br from-[#EC7134] to-[#E35F1E] -mx-6 -mt-6 px-6 py-5 rounded-t-xl mb-4 shadow-inner relative overflow-hidden">
            <div className="absolute inset-0 bg-grid-white/10 bg-[size:16px_16px] opacity-20"></div>
            <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-[#EC7134]/0 via-[#EC7134]/50 to-[#EC7134]/0"></div>
            <DialogTitle className="text-xl text-white font-medium mb-2 relative">Unlock Full Legal Protection</DialogTitle>
            <DialogDescription className="text-orange-50 relative">
        ...
        <div className="pt-2">
          <div className="bg-orange-50 rounded-xl p-4 mb-6 border border-[#F3EEE4]">
            <h4 className="font-medium text-[#E35F1E] text-sm mb-2">Premium Services:</h4>
            <RadioGroup value={serviceType} onValueChange={(value) => setServiceType(value as ServiceType)} className="mt-3 space-y-3">
              <div className={`relative flex items-start space-x-2 p-4 rounded-xl transition-all duration-200 ${
                serviceType === 'analysis' 
                  ? 'bg-white border-2 border-[#EC7134] shadow-md' 
                  : 'border border-gray-200 hover:border-orange-200 hover:bg-white/80'
              }`}>
                {serviceType === 'analysis' && (
                  <div className="absolute -top-1 -right-1 bg-[#EC7134] text-white text-xs px-2 py-0.5 rounded-full shadow-sm">
                    Popular
                  </div>
                )}
                ...
                <div className="grid gap-2 w-full">
                  <div className="flex justify-between items-center">
                    <Label htmlFor="analysis" className="font-medium text-base">Premium Analysis</Label>
                    <div className="flex flex-col items-end">
                      <span className="font-bold text-[#D55D20] text-lg">£29.00</span>
              ...
              <div className={`relative flex items-start space-x-2 p-4 rounded-xl transition-all duration-200 ${
                ...
                <div className="grid gap-2 w-full">
                  <div className="flex justify-between items-center">
                    <Label htmlFor="combined" className="font-medium text-base">Analysis + Tenancy Agreement Rewrite</Label>
                    <div className="flex flex-col items-end">
                      <span className="font-bold text-[#D55D20] text-lg">£48.00</span>
                      <span className="text-xs text-gray-500">One-time payment</span>
                    </div>
                  </div>
                  <p className="text-sm text-gray-600">
            ...
            <div className="mt-5 pt-3 border-t border-orange-100">
              <h5 className="font-medium text-xs text-[#D55D20] mb-2">Includes:</h5>
              <ul className="space-y-2 text-xs text-gray-600">
                {/* All services include these items */}
                <li className="flex items-start">
                  <svg className="w-3 h-3 text-[#EC7134] mr-1 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  <span>Full legal analysis with compliance checks</span>
                </li>
                <li className="flex items-start">
                  <svg className="w-3 h-3 text-[#EC7134] mr-1 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  <span>Unfair terms highlighted with explanation</span>
                </li>
                <li className="flex items-start">
                  <svg className="w-3 h-3 text-[#EC7134] mr-1 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  <span>Downloadable PDF report with legal references</span>
                </li>
                ...
                {serviceType === 'combined' && (
                  <>
                    <li className="flex items-start">
                      ...
                      <span>Tenant-friendly tenancy agreement rewrite in clear language</span>
                    </li>
                    <li className="flex items-start">
                      <svg className="w-3 h-3 text-[#EC7134] mr-1 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        <path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                      <span>Simplified legal language and better formatting</span>
                    </li>
                  </>
                )}
              </ul>
          ...
          <div className="mb-5 bg-gradient-to-r from-orange-50 via-white to-orange-50 p-4 rounded-xl border border-[#F3EEE4] shadow-sm">
            <div className="flex justify-between items-center">
              <div>
                <div className="text-sm font-medium text-gray-900 mb-0.5">
                  {serviceType === 'analysis' 
                    ? 'Premium Analysis' 
                    : 'Analysis + Tenancy Agreement Rewrite'}
                </div>
                <div className="text-xs text-gray-500">One-time payment, no recurring fees</div>
              </div>
              <div className="flex flex-col items-end">
                <div className="font-bold text-[#D55D20] text-lg">{getPrice(serviceType)}</div>
                <div className="text-xs text-green-600 flex items-center">
                  <svg className="w-3 h-3 mr-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  {serviceType === 'analysis' ? 'Money-back guarantee' : 'Best value option'}
                </div>
              </div>
            </div>
          </div>
